# -*- coding: utf-8 -*-
# Generated by Django 1.10.2 on 2016-12-09 15:39
from __future__ import unicode_literals

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('library_sample_shared', '0002_load_shared_data'),
    ]

    operations = [
        migrations.CreateModel(
            name='FileLibrary',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=200, verbose_name='Name')),
                ('file', models.FileField(upload_to='libraries/%Y/%m/%d/')),
            ],
        ),
        migrations.CreateModel(
            name='Library',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=200, unique=True, verbose_name='Name')),
                ('date', models.DateTimeField(auto_now_add=True, verbose_name='Date')),
                ('status', models.SmallIntegerField(default=0)),
                ('concentration', models.FloatField(verbose_name='Concentration')),
                ('dna_dissolved_in', models.CharField(max_length=255, verbose_name='DNA Dissolved in')),
                ('sample_volume', models.PositiveIntegerField(verbose_name='Sample Volume')),
                ('equal_representation_nucleotides', models.BooleanField(default=True, verbose_name='Equal Representation of Nucleotides')),
                ('sequencing_depth', models.PositiveIntegerField(verbose_name='Sequencing Depth')),
                ('comments', models.TextField(blank=True, null=True, verbose_name='Comments')),
                ('is_in_request', models.BooleanField(default=False, verbose_name='Is in request?')),
                ('is_pooled', models.BooleanField(default=False, verbose_name='Is pooled?')),
                ('barcode', models.CharField(max_length=9, verbose_name='Barcode')),
                ('index_i7', models.CharField(blank=True, max_length=8, null=True, verbose_name='Index I7')),
                ('index_i5', models.CharField(blank=True, max_length=8, null=True, verbose_name='Index I5')),
                ('dilution_factor', models.PositiveIntegerField(blank=True, null=True, verbose_name='Dilution Factor (facility)')),
                ('concentration_facility', models.FloatField(blank=True, null=True, verbose_name='Concentration (facility)')),
                ('date_facility', models.DateTimeField(blank=True, null=True, verbose_name='Date (facility)')),
                ('sample_volume_facility', models.PositiveIntegerField(blank=True, null=True, verbose_name='Sample Volume (facility)')),
                ('amount_facility', models.FloatField(blank=True, null=True, verbose_name='Amount (facility)')),
                ('size_distribution_facility', models.CharField(blank=True, max_length=200, null=True, verbose_name='Size Distribution (facility)')),
                ('comments_facility', models.TextField(blank=True, null=True, verbose_name='Comments (facility)')),
                ('enrichment_cycles', models.PositiveIntegerField(verbose_name='No. of Enrichment Cycles')),
                ('index_reads', models.PositiveSmallIntegerField(verbose_name='Index Reads')),
                ('mean_fragment_size', models.PositiveIntegerField(verbose_name='Mean Fragment Size')),
                ('qpcr_result', models.FloatField(blank=True, null=True, verbose_name='qPCR Result')),
                ('qpcr_result_facility', models.FloatField(blank=True, null=True, verbose_name='qPCR Result (facility)')),
                ('concentration_determined_by', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='library_sample_shared.ConcentrationMethod', verbose_name='Concentration Determined by')),
                ('concentration_determined_by_facility', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, related_name='+', to='library_sample_shared.ConcentrationMethod', verbose_name='Concentration Determined by (facility)')),
                ('files', models.ManyToManyField(related_name='files', to='library.FileLibrary')),
                ('index_type', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='library_sample_shared.IndexType', verbose_name='Index Type')),
            ],
            options={
                'verbose_name': 'Library',
                'verbose_name_plural': 'Libraries',
            },
        ),
        migrations.CreateModel(
            name='LibraryProtocol',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=150, verbose_name='Protocol')),
                ('provider', models.CharField(max_length=100, verbose_name='Provider')),
            ],
            options={
                'verbose_name': 'Library Protocol',
                'verbose_name_plural': 'Library Protocols',
            },
        ),
        migrations.CreateModel(
            name='LibraryType',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=200, verbose_name='Type')),
                ('library_protocol', models.ManyToManyField(related_name='library_protocol', to='library.LibraryProtocol', verbose_name='Library Protocol')),
            ],
            options={
                'verbose_name': 'Library Type',
                'verbose_name_plural': 'Library Types',
            },
        ),
        migrations.AddField(
            model_name='library',
            name='library_protocol',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='library.LibraryProtocol', verbose_name='Library Protocol'),
        ),
        migrations.AddField(
            model_name='library',
            name='library_type',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='library_type', to='library.LibraryType', verbose_name='Library Type'),
        ),
        migrations.AddField(
            model_name='library',
            name='organism',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='library_sample_shared.Organism', verbose_name='Organism'),
        ),
        migrations.AddField(
            model_name='library',
            name='read_length',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='library_sample_shared.ReadLength', verbose_name='Read Length'),
        ),
    ]
